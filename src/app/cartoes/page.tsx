...
import { getCreditCardPurchases, getCreditCards } from "@/lib/api";
import {
  createCreditCard,
...
export default async function CartoesPage() {
  ...
  const [cards, purchases] = await Promise.all([getCreditCards(session.user), getCreditCardPurchases(session.user)]);
  const activeCards = cards.filter((card) => card.active);
  const totalLimitCents = activeCards.reduce((total, card) => total + card.limitCents, 0);
  const openInvoiceCents = purchases.reduce(
    (total, purchase) => total + (purchase.installments[0]?.amountCents ?? 0),
    0
  );
